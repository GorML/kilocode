    ...
    Effect.gen(function* () {
      ...
      const state = yield* InstanceState.make(
        Effect.fn("KilocodeWatcher.state")(function* (ctx) {
          ...
          // route would otherwise build the stack. The ref must stay
          // structurally identical to the one the file/pty handlers use so the
          // LayerMap shares a single build per directory.
          yield* locations.contextEffect({ directory: AbsolutePath.make(ctx.directory) })
        }),
      )
